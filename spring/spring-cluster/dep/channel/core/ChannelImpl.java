...
import com.dunkware.spring.messaging.channel.core.ChannelHandler.ChannelSetter;
import com.dunkware.spring.messaging.channel.core.ChannelHandler.MessageHandler;
import com.dunkware.spring.messaging.channel.core.ChannelHandler.MessageReply;
import com.dunkware.spring.messaging.message.DunkMessage;
import com.dunkware.spring.messaging.message.DunkMessageHelper;
...
public class ChannelImpl implements Channel, KafkaByteHandler {
	...
	private List<DunkMessageInterceptor> messageInterceptors = new ArrayList<DunkMessageInterceptor>();
	...
	public DunkMessage sendReply(Object payload) throws ChannelException {
		...
		DunkMessageInterceptor interceptor = new DunkMessageInterceptor() {

			@Override
			public boolean intercept(DunkMessage message) {
				if (message.hasHeader(DunkMessage.HEADER_KEY_MESSAGE_REQUEST_ID)) {
					if (message.getHeader(DunkMessage.HEADER_KEY_MESSAGE_REQUEST_ID).equals(requestId)) {
						if (message.hasHeader(DunkMessage.HEADER_KEY_MESSAGE_RESPONSE_ERROR)) {
							responseQueue.add(message.getHeader(DunkMessage.HEADER_KEY_MESSAGE_RESPONSE_ERROR));
						} else {
		...
		try {
			...
			return DunkMessage.newInstance(response);
		} catch (Exception e) {
	...
	@Override
	public void addMessageInterceptor(final DunkMessageInterceptor interceptor) {
	...
	@Override
	public void removeMessageInterceptor(DunkMessageInterceptor interceptor) {
		...
		public void run() {
			while (!interrupted()) {
				try {
					...
					DunkMessageTransport transport = inboundMessages.take();
					DunkMessage message = null;
					try {
						message = DunkMessage.newInstance(transport);
					} catch (Exception e) {
					...
					if(message.hasHeader(DunkMessage.HEADER_KEY_MESSAGE_TYPE)) {
						if(message.getHeader(DunkMessage.HEADER_KEY_MESSAGE_TYPE).equals(DunkMessage.HEADER_MESSAGE_TYPE_REQUEST)) { 
							...
							try {
								...
								for (ChannelHandler channelHandler : channelHandlers) {
									if(channelHandler.hasMessageReply(payload)) { 
										try {
											...
											DunkMessage responseMessage = DunkMessage.newInstance(response);
											responseMessage.setHeader(DunkMessage.HEADER_KEY_MESSAGE_TYPE, DunkMessage.HEADER_REQUEST_TYPE_RESPONSE);
											responseMessage.setHeader(DunkMessage.HEADER_KEY_MESSAGE_REQUEST_ID, message.getHeader(DunkMessage.HEADER_KEY_MESSAGE_ID));
											send(responseMessage);
											handled = true; 
										} catch (Exception e) {
											DunkMessage responseMessage = DunkMessage.newInstance();
											responseMessage.setHeader(DunkMessage.HEADER_KEY_MESSAGE_TYPE, DunkMessage.HEADER_REQUEST_TYPE_RESPONSE);
											responseMessage.setHeader(DunkMessage.HEADER_KEY_MESSAGE_REQUEST_ID, message.getHeader(DunkMessage.HEADER_KEY_MESSAGE_ID));
											responseMessage.setHeader(DunkMessage.HEADER_KEY_MESSAGE_RESPONSE_ERROR, e.toString());
											try {
												send(responseMessage);
												handled = true;
											} catch (Exception e2) {
								...
								if(!handled) {
									...
									responseMessage.setHeader(DunkMessage.HEADER_KEY_MESSAGE_TYPE, DunkMessage.HEADER_REQUEST_TYPE_RESPONSE);
									responseMessage.setHeader(DunkMessage.HEADER_KEY_MESSAGE_REQUEST_ID, message.getHeader(DunkMessage.HEADER_KEY_MESSAGE_ID));
									responseMessage.setHeader(DunkMessage.HEADER_KEY_MESSAGE_RESPONSE_ERROR, "No handlers found");
									try {
										send(responseMessage);
									} catch (Exception e2) {
										logger.error("Exception sending response error message " + e2.toString());
					...
					try {
						messageInterceptorLock.acquire();
						for (DunkMessageInterceptor interceptor : messageInterceptors) {
