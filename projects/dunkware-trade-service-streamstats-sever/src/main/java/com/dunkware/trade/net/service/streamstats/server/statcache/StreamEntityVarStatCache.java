...
import com.dunkware.trade.net.service.streamstats.server.service.core.StreamStatsHelper;
...
public class StreamEntityVarStatCache {
	...
	public void consumeSession(Document doc, DDate date) { 
		...
		session.setHigh(StreamStatsHelper.stringToNumber(high, dataType));
		session.setLow(StreamStatsHelper.stringToNumber(low, dataType));
	...
	public StreamEntityVarSession resolveRelativeHigh(DDate date, int days) throws StreamStatsResolveException { 
		checkSort();
		StreamEntityVarSession startSession = getFirstRelativeSession(date);
		int startIndex = sessions.indexOf(startSession);
		if((sessions.size() - startIndex) < days) { 
			throw new StreamStatsResolveException("Not enugh session for days " + days);
		}
	...
	private StreamEntityVarSession getFirstRelativeSession(DDate date) throws StreamStatsResolveException { 

		StreamEntityVarSession resolved = null;
		for (StreamEntityVarSession session : sessions) {
			...
			if(session.getDate().isAfter(resolved.getDate())) { 
				resolved = session;
			}
		}
		if(resolved == null) { 
			throw new StreamStatsResolveException("Cannot find recent date that is before target date " + date.toString());
		}
		return resolved;
