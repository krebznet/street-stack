...
public class MockSnapshotPublisher {
	...
	public void start(String sessionId, String brokers, String topic) throws Exception {
		...
		try {
			...
			throw new Exception("Exception connecting mock snapshot kafka producer " + e.toString(), e);
		}
		...
		while(queue.isEmpty() == false) { 
			try {
				...
				if(loopCount > 10) { 
					...
					logger.error(MockMarker.getMarker(), "Disposing Snapshot Publisher Disposing with " + queue.size() + " pending envents");
				}
			} catch (Exception e) {
			}
		}
	...
	public void publish(MockSessionEntity entity, MockSession session) {
		GStreamEvent event = MockProtoUtil.entitySnapshotEvent(entity, session);
		try {
			queue.put(event);
		} catch (Exception e) {
			logger.error(MockMarker.getMarker(), "Exception putting snapshot event in publish q " + e.toString());
		}
	...
	private class Publisher extends Thread { 
		...
		public void run() { 
			try {
				while(!interrupted()) { 
					try {
						...
						try {
							...
							logger.error(MockMarker.getMarker(), "Exception publishing snapshot " + e.toString(),e);
						}
					} catch (Exception e) {
						if (e instanceof InterruptedException) {
							return;
						}
						logger.error(MockMarker.getMarker(), "Exception taking signal to snapshot " + e.toString(),e);
				...
				logger.error(MockMarker.getMarker(), "Outer snapshot Publisher Exception " + e.toString());
			}
		}
	}
