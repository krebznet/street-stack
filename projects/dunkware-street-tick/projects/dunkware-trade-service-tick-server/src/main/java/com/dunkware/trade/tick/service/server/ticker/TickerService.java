...
public class TickerService  {
	...
	@Autowired
	private TickerListSubscribeRepo tickerListSubscribeRepo;
	...
	@PostConstruct
	private void load() {
		logger.info("Loading Ticker Service"); 
			...
			while (rs.next()) {
				...
				try {
					...
					if (ticker.isPresent() == false) {
						throw new Exception(
								"Ticker query by symbol name " + symbol + " id " + symbolId + "returned nothing");
					}
	...
	public TickerDO findTIckerBySymbol(String symbol) throws Exception { 
		List<TickerDO> results = repo.findBySymbol(symbol);
		if(results.size() == 0) { 
			return null;
		}
