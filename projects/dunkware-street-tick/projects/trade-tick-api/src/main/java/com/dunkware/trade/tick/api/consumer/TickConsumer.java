...
public class TickConsumer {
	...
	public TickConsumerSession start(TickConsumerSpec spec, TickFeed feed, String kafkaBrokers, String topicPrefix)
		...
		String sessionid = ("tick_feed_" + spec.getName() + "_" + DunkUUID.randomUUID(4));
		session.setSessionId(sessionid);
		session.setKafkaBroker(kafkaBrokers);
		session.setKafkaTopic(topicPrefix + "_" + sessionid);

		try {
			kafkaProducer = KafkaByteProducer.newInstance(kafkaBrokers, session.getKafkaTopic(), DunkUUID.randomUUID(5));
			

			DunkKafkaAdmin admin = DunkKafkaAdmin.newInstance(kafkaBrokers);
		KafkaNewTopic newTopic =	KafkaNewTopic.builder().name(session.getKafkaTopic()).paritions(1).replicas((short)1).cleanupPolicy(TopicConfig.CLEANUP_POLICY_DELETE).retentionTime(3, TimeUnit.DAYS).paritions(1).build();
			
			KafkaNewTopicResult result = admin.createTopic(newTopic);
	...
	public void clientHeartbeat() {
		lastHeartbeat = DunkTimeZones.dateTimeNewYork();
	}
