...
import com.dunkware.xstream.data.capture.snapshot.MongoSnapshotBucket;
import com.dunkware.xstream.util.XStreamEventException;

public class MongoCaptureHelper {
	 ...
	 * @throws Exception
	 */
	public static Document buildSnapshotBucket(MongoSnapshotBucket bucket, DTimeZone timeZone) throws Exception { 
		...
		for (GEntitySnapshot snapshot : bucket.getSnapshots()) {
			...
			for (GEntitySnapshot.GSnapshotSignal sig : snapshot.getSignalsList()) {
				...
				sd.append("name", sig.getIdentifier());
	...
	public static List<Document> buildVarSnapshots(List<GEntityVarSnapshot> snapshots) throws Exception{
		...
		for (GEntityVarSnapshot var : snapshots) {
			...
			varDoc.append("name", var.getIdentifier());
			boolean handled = false;
