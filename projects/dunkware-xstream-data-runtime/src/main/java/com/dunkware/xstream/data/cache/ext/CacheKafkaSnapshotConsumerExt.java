...
@ACacheExtension(type = CacheKafkaSnapshotConsumerExtType.class)
public class CacheKafkaSnapshotConsumerExt implements CacheExtension, DKafkaByteHandler2 {

	
	private Logger logger = LoggerFactory.getLogger(getClass());
	
	private CacheStream cache; 
	private CacheKafkaSnapshotConsumerExtType type;
	
	private DKafkaByteConsumer2 consumer;
	
	private AtomicLong snapshotCount = new AtomicLong();
	
	private BlockingQueue<GEntitySnapshot> snapshotQueue = new LinkedBlockingQueue<GEntitySnapshot>();
	...
	public void record(ConsumerRecord<String, byte[]> record) {
		try {
			...
			if(event.getType() != GStreamEventType.EntitySnapshot) { 
				...
				snapshotQueue.put(event.getEntitySnapshot());
			}
		} catch (Exception e) {
	  ...
	  class SnapshotPublisher extends Thread { 
			...
			public void run() { 
				while(!interrupted()) { 
					try {
						GEntitySnapshot snapshot = snapshotQueue.take();
						// okay we need time 
						...
						try {
							...
							cache.insertSnapshot(snapshot.getId(), snapshot.getIdentifier(),dt,valueSet);
