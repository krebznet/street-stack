...
@Service
public class StreamEntityStatsProviderImpl implements StreamEntityStatsProvider  {
	...
	private List<StreamEntityStats> streamStats = new ArrayList<StreamEntityStats>();
	...
	private void init() { 
		...
		for (StreamDataProvider provider: streamProviders.getProviders()) {
			StreamEntityStatsImpl streamStats = new StreamEntityStatsImpl();
	...
	public StreamEntityStats getStream(String streamIdentifier) throws Exception {
		for (StreamEntityStats stats : streamStats) {
