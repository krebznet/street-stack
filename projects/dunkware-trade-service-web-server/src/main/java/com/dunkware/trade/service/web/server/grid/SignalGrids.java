...
public class SignalGrids {
	
	
	public static Grid grid1(GList entities) throws Exception { 
		GridBuilder builder = GridBuilder.newBuilder();
		builder.addColumn("id").setHeaderName("Symbol").add().addColumn("signal").setHeaderName("Signal").add().addColumn("time").setHeaderName("Time").add()
		.addColumn("date").setHeaderName("Date").add().
		addColumn("TickLast").setHeaderName("Last Price").add();
