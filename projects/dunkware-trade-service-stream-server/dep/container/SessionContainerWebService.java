...
public class SessionContainerWebService implements StreamingListener {
	...
	public @ResponseBody() SessionEntityScannerStartResp entityScannerStart(@RequestBody() SessionEntityScanner req) { 
		...
		try {
			scanner.start(streamingAdapter,req,container,scannerIdentifier);
		} catch (Exception e) {
			SessionEntityScannerStartResp resp = new SessionEntityScannerStartResp();
			resp.setSuccess(false);
