...
public class DZonedClockUpdater {
	...
	public static DZonedClockUpdater now(DTimeZone timeZone, long updateInterval, TimeUnit updateUnit) { 
		...
		public void run() { 
			while(!interrupted()) { 
				try {
					...
					clock.setDateTime(DZonedDateTime.now(clock.getDateTime().getTimeZone()));
				} catch (Exception e) {
					disposed = true;
				}
			}
