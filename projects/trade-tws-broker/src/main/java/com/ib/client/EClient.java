...
public abstract class EClient {
    ...
    public synchronized void placeOrder( int id, Contract contract, TwsOrder order) {
        // not connected?
        if( !isConnected()) {
            notConnected();
            return;
        ...
        if (m_serverVersion < MIN_SERVER_VER_ORDER_COMBO_LEGS_PRICE && SecType.BAG.name().equalsIgnoreCase(contract.getSecType())) {
        	if (!order.orderComboLegs().isEmpty()) {
                for( TwsOrderComboLeg orderComboLeg : order.orderComboLegs() ) {
        ...
        try {
           ...
           if (m_serverVersion >= MIN_SERVER_VER_PEGGED_TO_BENCHMARK) {
        	   if (order.orderType() == TwsOrderType.PEG_BENCH) {
        	   ...
        	   if (order.conditions().size() > 0) {
        		   for (TwsOrderCondition item : order.conditions()) {
