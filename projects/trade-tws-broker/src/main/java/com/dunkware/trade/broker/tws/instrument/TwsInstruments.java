...
/**
 * Put the instrument and quote shit in its own class
 * instead of stuffing it in TwsBroker 
 * 
 * @author dkrebs
 *
 */
public class TwsInstruments implements TwsSocketReader {
	...
	private ConcurrentHashMap<TradeTickerSpec, Instrument> instruments = new ConcurrentHashMap<TradeTickerSpec, Instrument>();
	...
	private ConcurrentHashMap<Integer, ReusableCountDownLatch> pendingSubscriptions = new ConcurrentHashMap<Integer, ReusableCountDownLatch>();;
	...
	public Instrument subscribe(TradeTickerSpec ticker) throws Exception { 
		if(ticker.getType() != TradeTickerType.Equity) { 
			throw new Exception("Trade Ticker Type " + ticker.getType().toString() + " not handled in tws");
		}
		StkContract contract = new StkContract(ticker.getSymbol());
		int id = instrumentIdFactory.incrementAndGet();
		instrumentIds.put(id, ticker);
		instrumentInverseIds.put(ticker, id);
		...
		// remove pending subscription latch
	...
	/**
	 * Called on every tick update so we can return any threads blocking
	 * while new subscriptions are creating. 
	 * @param equity
	 */
	private void processPending(int instrumentId, Instrument instrument) { 
		if(pendingSubscriptions.containsKey(instrumentId)) { 
			if(isProcessed(instrument)) { 
				ReusableCountDownLatch latch = pendingSubscriptions.get(instrumentId);
				latch.decrement();
			}
		}
	}
	...
	private boolean isProcessed(Instrument instrement) { 
		if(instrement.getAskPrice() != Double.MIN_VALUE) { 
			if(instrement.getBidPrice() !=  Double.MIN_VALUE) {
				if(instrement.getAskSize() != Integer.MIN_VALUE) { 
					if(instrement.getBidSize() != Integer.MIN_VALUE) { 
						if(instrement.getLast() != Double.MIN_VALUE) { 
							return true; 
						}
					}
				}
