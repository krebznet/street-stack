...
public class DataStreamSnapshotWriter implements DKafkaByteHandler2 {

	private Logger logger = LoggerFactory.getLogger(getClass());

	private Map<String, DataStreamSnapshotWriterBucket> buckets = new ConcurrentHashMap<String, DataStreamSnapshotWriterBucket>();

	private BlockingQueue<DataStreamSnapshotWriterBucket> writeQueue = new LinkedBlockingQueue<DataStreamSnapshotWriterBucket>();
	...
	private DataStreamSnapshotWriterInput input;
	...
	public DataStreamSnapshotWriter() {

	}
	

	public void start(DataStreamSnapshotWriterInput input) throws Exception {
		this.input = input;
	...
	public void record(ConsumerRecord<String, byte[]> record) {
		...
		if (buckets.get(snapshot.getIdentifier()) == null) {
			DataStreamSnapshotWriterBucket bucket = new DataStreamSnapshotWriterBucket();
			...
			DataStreamSnapshotWriterBucket bucket = buckets.get(snapshot.getIdentifier());
			bucket.addSnapshot(snapshot);
			buckets.put(snapshot.getIdentifier(), bucket);
	...
	private class BucketWriter extends Thread { 
		...
		public void run() { 
			while(!interrupted()) { 
				DataStreamSnapshotWriterBucket bucket = null;
				try {
				...
				try {
					...
					Document document = DataStreamWriterHelper.buildSnapshotBucket(bucket,input.getTimeZone());
