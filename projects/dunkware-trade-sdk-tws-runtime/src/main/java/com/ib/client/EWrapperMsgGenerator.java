...
public class EWrapperMsgGenerator {
    ...
    public static String openOrder( int orderId, Contract contract, TwsOrder order, TwsOrderState orderState) {
    ...
    public static String completedOrder( Contract contract, TwsOrder order, TwsOrderState orderState) {
        final StringBuilder sb = new StringBuilder(1024);
    ...
    private static void appendOrderFields(StringBuilder sb, int orderId, Contract contract, TwsOrder order, TwsOrderState orderState,
            boolean isOpenOrder) {
            ...
            if (contract.comboLegs() != null) {
                for (int i = 0; i < contract.comboLegs().size(); ++i) {
                    ...
                    if (order.orderComboLegs() != null && contract.comboLegs().size() == order.orderComboLegs().size()) {
                        TwsOrderComboLeg orderComboLeg = order.orderComboLegs().get(i);
        ...
        if (order.conditions() != null && order.conditions().size() > 0) {
            ...
            for (TwsOrderCondition condition : order.conditions()) {
