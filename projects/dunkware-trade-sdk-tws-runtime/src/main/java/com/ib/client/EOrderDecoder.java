...
public class EOrderDecoder {
    ...
    private final TwsOrderState m_orderState;
    private final int m_version;
    private final int m_serverVersion;

    EOrderDecoder(EDecoder eDecoder, Contract contract, TwsOrder order, TwsOrderState orderState, int version, int serverVersion) {
        m_eDecoder = eDecoder;
        m_contract = contract;
        m_order = order;
        ...
        if (m_version >= 29) {
            ...
            if (orderComboLegsCount > 0) {
                ...
                for (int i = 0; i < orderComboLegsCount; ++i) {
                    ...
                    TwsOrderComboLeg orderComboLeg = new TwsOrderComboLeg(price);
    ...
    public void readPegToBenchParams() throws IOException {
        if (m_serverVersion >= EClient.MIN_SERVER_VER_PEGGED_TO_BENCHMARK) {
            if (m_order.orderType() == TwsOrderType.PEG_BENCH) {
        ...
        if (m_serverVersion >= EClient.MIN_SERVER_VER_PEGGED_TO_BENCHMARK) {
            ...
            if (nConditions > 0) {          
                for (int i = 0; i < nConditions; i++) {
                    TwsOrderConditionType orderConditionType = TwsOrderConditionType.fromInt(m_eDecoder.readInt());              
                    TwsOrderCondition condition = TwsOrderCondition.create(orderConditionType);
    ...
    public void readAdjustedOrderParams() throws IOException {
        if (m_serverVersion >= EClient.MIN_SERVER_VER_PEGGED_TO_BENCHMARK) {
            m_order.adjustedOrderType(TwsOrderType.get(m_eDecoder.readStr()));
