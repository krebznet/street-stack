...
public class ApiController implements EWrapper {
	...
	public interface IOrderHandler {
		void orderState(TwsOrderState orderState);
		void orderStatus(TwsOrderStatus status, Decimal filled, Decimal remaining, double avgFillPrice, int permId, int parentId, double lastFillPrice, int clientId, String whyHeld, double mktCapPrice);
	...
	public void placeOrModifyOrder(Contract contract, final TwsOrder order, final IOrderHandler handler) {
		if (!checkConnection())
			return;
	...
	public interface ILiveOrderHandler {
		void openOrder(Contract contract, TwsOrder order, TwsOrderState orderState);
		void openOrderEnd();
		void orderStatus(int orderId, TwsOrderStatus status, Decimal filled, Decimal remaining, double avgFillPrice, int permId, int parentId, double lastFillPrice, int clientId, String whyHeld, double mktCapPrice);
	...
	@Override public void openOrder(int orderId, Contract contract, TwsOrder order, TwsOrderState orderState) {
	...
	@Override public void orderStatus(int orderId, String status, Decimal filled, Decimal remaining, double avgFillPrice, int permId, int parentId, double lastFillPrice, int clientId, String whyHeld, double mktCapPrice) {
		IOrderHandler handler = m_orderHandlers.get( orderId);
		if (handler != null) {
			handler.orderStatus( TwsOrderStatus.valueOf( status), filled, remaining, avgFillPrice, permId, parentId, lastFillPrice, clientId, whyHeld, mktCapPrice);
		}
		...
		for (ILiveOrderHandler liveOrderHandler : m_liveOrderHandlers) {
			liveOrderHandler.orderStatus(orderId, TwsOrderStatus.valueOf( status), filled, remaining, avgFillPrice, permId, parentId, lastFillPrice, clientId, whyHeld, mktCapPrice);
		}
		recEOM();
	}
    ...
    public interface ICompletedOrdersHandler {
        void completedOrder(Contract contract, TwsOrder order, TwsOrderState orderState);
