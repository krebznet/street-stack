...
import com.ib.client.TwsOrderStatus;
import com.ib.client.TwsOrderType;
...
class TicketDlg extends JDialog {
	...
	private final TwsOrder m_order;
    ...
    TicketDlg(Contract contract, TwsOrder order) {
        this(contract, order, false);
    }

    TicketDlg(Contract contract, TwsOrder order, boolean cancel) {
	...
	private void onTransmitOrder() {
		...
		ApiDemo.INSTANCE.controller().placeOrModifyOrder( m_contract, m_order, new IOrderHandler() {
			@Override public void orderState(TwsOrderState orderState) {
				ApiDemo.INSTANCE.controller().removeOrderHandler( this);
				SwingUtilities.invokeLater(() -> dispose());
			}
			@Override public void orderStatus(TwsOrderStatus status, Decimal filled, Decimal remaining, double avgFillPrice, int permId, int parentId, double lastFillPrice, int clientId, String whyHeld, double mktCapPrice) {
			}
	...
	private void onCheckMargin() {
		...
		m_order.whatIf( true);
		ApiDemo.INSTANCE.controller().placeOrModifyOrder( m_contract, m_order, new IOrderHandler() {
			@Override public void orderState(final TwsOrderState orderState) {
	...
	private void displayMargin(TwsOrderState orderState) {
	...
	class OrderPanel extends VerticalPanel {
		...
		final TCombo<TwsOrderType> m_orderType = new TCombo<>( TwsOrderType.values() );
	...
	class VolatilityTicketPanel extends VerticalPanel {
		...
		final TCombo<TwsOrderType> m_deltaNeutralOrderType = new TCombo<>( TwsOrderType.values() );
