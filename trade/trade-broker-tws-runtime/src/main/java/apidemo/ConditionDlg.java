...
import com.ib.client.TwsOrderConditionType;
...
public class ConditionDlg extends JDialog implements ChangeListener, ActionListener {
    ...
    private TwsOrderCondition m_condition;
	private OnOKPanel m_conditionSubPanel;
	
	private ContractLookuper m_lookuper;
    
    ConditionDlg(TwsOrderCondition condition, ContractLookuper lookuper) {
        initComponents();
	...
	private TwsOrderCondition instantiateCondition(TwsOrderConditionType type) {
		if (m_condition.type() != type) {
            m_condition = TwsOrderCondition.create(type);
        }
		return m_condition;
	}
	
	public TwsOrderCondition condition() {
		return m_condition;
	}
