package com.dunkware.trade.service.stream.serverd.controller.session.extensions;
...
@AStreamSessionExt
public class EntityStatsTopicCreator implements StreamSessionExtension {
	...
	public void sessionStarting(StreamSession session) {
		try {
			...
			session.getInput().getProperties().put("stats_topic", getStatTopicName(session.getSessionEntityId()));
			this.session = session;
