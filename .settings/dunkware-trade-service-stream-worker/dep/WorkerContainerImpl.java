...
import com.dunkware.trade.service.stream.worker.session.container.scanners.WorkerEntitySearch;
import com.dunkware.xstream.net.core.container.Container;
import com.dunkware.xstream.net.core.container.ContainerBuilder;


public class WorkerContainerImpl {
	...
	private WorkerContainerStartReq req;

	@Autowired
	private Cluster cluster;

	private KafkaByteConsumer workerConsumer;
	private KafkaByteConsumer workersConsumer;
	private KafkaByteProducer serverProducer;
	...
	public void start(WorkerContainerStartReq req) throws Exception {
		...
		try {
			KafkaByteConsumerSpec spec = KafkaByteConsumerSpecBuilder
					...
					.setClientAndGroup("DataContainerWorker_" + req.getWorkerId() + "_" + DunkUUID.randomUUID(4),
							"DataContainerWorker_" + req.getWorkerId() + "_" + DunkUUID.randomUUID(4))
					.build();
			workerConsumer = KafkaByteConsumer.newInstance(spec);
		...
		try {
			serverProducer = KafkaByteProducer.newInstance(req.getKafkaBroker(), req.getServerTopic(),
					req.getWorkerId() + "_" + DunkUUID.randomUUID(4));
		} catch (Exception e) {
	...
	private class WorkerMessageConsumer implements KafkaByteHandler {
	...
	public WorkerContainerStartReq getReq() {
		return req;
	}
		...
		public void run() {
			while (!interrupted()) {
				...
				try {
					...
					if(message.getTypeCase() == TypeCase.ENTITYSCANNERSTARTREQUEST) { 
						GNetEntityScannerStartRequest startReq = message.getEntityScannerStartRequest();
						
					}
					
					if(message.getTypeCase() == TypeCase.ENTITYSEARCHREQUEST2) {
						GNetEntitySearchRequest2 req = message.getEntitySearchRequest2();
						//WorkerEntitySearch search = new WorkerEntitySearch(req,WorkerContainerImpl.this);
