...
public class AsyncQueryBuilder  {
	...
	public Future<XStreamEntityQuery> buildQuery(AsyncQueryContext context, XStreamEntityQueryType model) {
		...
		for (XStreamEntityCriteriaType criteria : model.getCriterias()) {
	...
	private Future<XStreamEntityPredicate> buiildPredicate(XStreamEntityCriteriaType model) {
		...
		if (model.getType() == XStreamEntityCriteriaKind.ValueCompare) {
	...
	private Future<XStreamEntityQueryValue> buildEntityValue(XStreamEntityValueType model) {
		if (model.getType() == XStreamEntityValueKind.SignalHistoricalCount) {
			return buildSignalHistCountValue(model);

		}
		if (model.getType() == XStreamEntityValueKind.SignalSessionCount) {
			return buildSignalCountValue(model);
		}
		if (model.getType() == XStreamEntityValueKind.VarCurrentValue) {
			return buildVarCurrentValue(model);
		}

		if (model.getType() == XStreamEntityValueKind.VarHistoricalAgg) {
			return buildVarAggHistValue(model);
		}

		if (model.getType() == XStreamEntityValueKind.VarSessionAgg) {
	...
	private Future<XStreamEntityQueryValue> buildVarAggHistValue(XStreamEntityValueType model) {
		Promise<XStreamEntityQueryValue> promise = Promise.promise();
	...
	private Future<XStreamEntityQueryValue> buildVarCurrentValue(XStreamEntityValueType model) {
		Promise<XStreamEntityQueryValue> promise = Promise.promise();
		Future<XStreamEntityQueryValue> future = promise.future();
		model.getVarIdent();
	...
	private Future<XStreamEntityQueryValue> buildVarAggValue(XStreamEntityValueType model) {
		Promise<XStreamEntityQueryValue> promise = Promise.promise();
		Future<XStreamEntityQueryValue> future = promise.future();
		return future;
	...
	private Future<XStreamEntityQueryValue> buildSignalCountValue(XStreamEntityValueType model) {
		Promise<XStreamEntityQueryValue> promise = Promise.promise();
		Future<XStreamEntityQueryValue> future = promise.future();
	...
	private Future<XStreamEntityQueryValue> buildSignalHistCountValue(XStreamEntityValueType model) {
		Promise<XStreamEntityQueryValue> promise = Promise.promise();
		Future<XStreamEntityQueryValue> future = promise.future();
		promise.fail("Signal Counts Not Implemented Yet");
		return future;
